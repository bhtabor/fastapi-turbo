...
All entries below are relative to that version.
...
- Changed: `render_stream(...)` and the new `render_string(...)` render
  template files; `forms.validation_error_stream`
...
- Added: cache correctness for header-varied responses — `TurboStreamResponse`
  sets `Vary: Accept`, and the merging `append_vary(headers, value)` helper
  is exported. Frame negotiation follows the turbo-rails model: endpoints
  branch on `TurboContext.is_frame_request` to serve one URL as either a
  full page or a frame fragment, keeping the two bodies apart by varying
  the response ETag on frame-ness.
- Removed: `TurboContext.is_visit` (previously derived from
  `Sec-Fetch-Mode`).
- Removed: session-backed flash helpers (`flash`, `get_flashed`,
  `FlashMessage`, the automatic `flashes` context processor,
  `examples/flash`). `TurboTemplates` no longer touches
  `request.session`; apps needing flash messaging own that plumbing.
- Added: `streams.SELECTOR_ACTIONS` — the built-in actions
  (`after`/`append`/`before`/`prepend`/`remove`/`replace`/`update`) whose
  named builders require `target=` or `targets=`. `TurboTemplates.render_stream`
  now enforces the same requirement for those actions, matching the named
  builders; custom actions and `refresh` remain unchecked.
