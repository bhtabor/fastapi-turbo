...
| [`TurboTemplates`](#turbotemplates) | A `Jinja2Templates` wrapper that adds `render_stream(...)` and `render_string(...)`, plus an automatic `turbo_script` template global. |
...
def item_frame(
    ...
    # turbo-rails model: one URL, two bodies. Frame requests get the
    # partial alone; full-page navigations get the laid-out page. It's
    # just template selection — no special response helper.
    name = "partials/item.html" if turbo.is_frame_request else "pages/item.html"
    return templates.TemplateResponse(request, name, {"item": load(id)})
...
Frame negotiation is pure template selection — `TurboTemplates` adds no fragment helper for it. Keep the two bodies apart by varying the response ETag on frame-ness (as turbo-rails does), so a revalidating cache never serves one body for the other.
